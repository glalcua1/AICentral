...
interface ProjectSubmissionModalProps {
  isOpen: boolean
  onClose: () => void
  project?: Project // Optional project for editing mode
}
...
export default function ProjectSubmissionModal({ isOpen, onClose, project }: ProjectSubmissionModalProps) {
  ...
  useEffect(() => {
    if (project && isOpen) {
      setFormData({
        ...
        link: project.link || '',
        screenshots: [] // Keep screenshots empty for editing, as File objects can't be recreated
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (project) {
        ...
        const updatedData = {
          ...
          screenshots: formData.screenshots.length > 0 
            ? formData.screenshots.map(file => URL.createObjectURL(file))
            : project.screenshots, // Keep existing screenshots if no new ones uploaded
        }
        ...
        const submissionData = {
          ...
          submittedBy: 'jennifer.kim@company.com', // In real app, this would come from auth
      ...
      alert(`There was an error ${project ? 'updating' : 'submitting'} your project. Please try again.`)
      setIsSubmitting(false)
    }
  }
        ...
        <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-2xl">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="heading-3 text-gray-900">{project ? 'Edit AI Project' : 'Submit AI Project'}</h2>
